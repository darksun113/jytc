  ...
  <div :id="id" :class="['vue-puzzle-vcode', { show_: show }]" @mousedown="onCloseMouseDown" @mouseup="onCloseMouseUp" @touchstart="onCloseMouseDown" @touchend="onCloseMouseUp">
    <div class="vue-auth-box_" :style="{'border-radius':borderRadius+'px'}" @mousedown.stop @touchstart.stop>
      <div class="auth-body_" :style="`height: ${canvasHeight}px`">
        ...
        <!-- 小图 -->
        <img ref="img2" :src="image2" class="auth-canvas2_" :height="canvasHeight" :style="`transform:translateX(${styleWidth - sliderBaseSize}px)`" alt="">
        ...
        <div :class="['flash_', { show: isSuccess }]" :style="`transform: translateX(${isSuccess ? `${canvasWidth + canvasHeight * 0.578}px` : `-${canvasHeight * 0.578}px`}) skew(-30deg, 0);`" />
...
export default {
  ...
  props: {
    ...
    canvasWidth: { type: Number, default: 300 }, // 主canvas的宽
  ...
  methods: {
    ...
    async getPuzzleData(){
      try {
        const res = await uni.$http("/puzzle/get", { type: 1})
        if(res.code == 0){
    ...
    async check(result,x){
      try {
        const res = await uni.$http("/puzzle/verify",{x,id:this.roundId})
        if (res.data) {
            // 成功
