<template>
	<view class="muqian-content" :style="{
		...
		<!-- 加载中 -->
		<image :src="loadSrc" class="muqian-image muqain-load" @load="init"  :mode="mode" :style="{
				opacity:isShow?'0':'1',
				borderRadius,
				width,
				height,
		...
		<!-- 加载成功 -->
		<image class="muqian-image" @load="load" @error="error" v-if="status==1" :src="src" :mode="mode" :style="{
				opacity:isShow?'1':'0',
				borderRadius,
				width,
				height,
		...
		<!-- 加载失败 -->
		<image class="muqian-image" v-if="status==2" :src="errorSrc"  :mode="mode" :style="{
			opacity:isShow?'1':'0',
			borderRadius,
			width,
			height,
